<?php 
    ...
    function nepali_to_english($nepali=null){
        list($nepali_y, $nepali_m, $nepali_d) = @explode("-",$nepali);
        $cal = new Nepali_Calendar();
        $nepdate = $cal->nep_to_eng($nepali_y, $nepali_m, $nepali_d);
        return $nepdate;
    }

    function age_calculator($output){
        $diff = date_diff(date_create($output),date_create(date('Y-m-d')));
        return array('y' => $diff->y, 'm' => $diff->m, 'd' => $diff->d );
    }

    function format_date($output){
        return $output['year'].'-'.$output['month'].'-'.$output['date'];
    }


    if(isset($_POST['nepali'])){
        $date = format_date(nepali_to_english($_POST['nepali']));
        echo json_encode(array('status'=>'success', 'date'=>$date, 'age' => age_calculator($date)));exit;

    }elseif($_POST['english']){
        $date = nepali_to_english($_POST['english']);
        echo json_encode(array('status'=>'success', 'date'=>$date, 'age' => age_calculator($date)));exit;
    }else{
        $date = date('y-m-d');
        echo json_encode(array('status'=>'success', 'date'=>$date, 'age' => age_calculator($date)));exit;
